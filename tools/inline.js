...
(function(){
  ...
  (function(){
    ...
    Object.defineProperty(SP,'src',{ configurable:true, enumerable:true,
      ...
      set:function(v){ var k=find(v); var r=k?F[k]:null; sd.set.call(this, r ? blobURL(r) : v); }
    });
  })();
})();
</script>`;
...
(function(){
  ...
  // No pre-registration: asset-bundle index.js files are loaded by Cocos via a real
  // <script> tag, which the script-src interceptor serves from the map as a blob URL.
  // That lets Cocos run its normal bundle setup (so the start scene loads) on ANY host
  // path. Pre-registering them here would make Cocos think the bundle is already loaded
  // and skip that setup -> the scene never loads on a subpath.
})();
</script>`;
